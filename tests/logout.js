...
test(`${path.basename(
    __filename,
)}: Logging out with all=false only logs out the current session`, async t => {
    t.plan(11);
    ...
    t.truthy(
        (await loggedInClientUser1.query(Get(t.context.testDocumentRef))).data,
    );
    t.truthy(
        (await loggedInClientUser2.query(Get(t.context.testDocumentRef))).data,
    );
    t.truthy(
        (await loggedInClientOtherUser.query(Get(t.context.testDocumentRef)))
            .data,
    );
    ...
    await t.throwsAsync(
        async () => {
            console.log(
                await loggedInClientUser1.query(Get(t.context.testDocumentRef)),
            );
        },
...
test(`${path.basename(
    __filename,
)}: Logging out with all=true logs out all sessions for that account`, async t => {
    t.plan(11);
    const client = t.context.databaseClients.childClient;
    const loginResultUserSession1 = await client.query(
        Call('login', 'user@domain.com', 'verysecure'),
    ...
    await t.throwsAsync(
        async () => {
            console.log(
                await loggedInClientUser2.query(Get(t.context.testDocumentRef)),
            );
        },
        { instanceOf: fauna.errors.Unauthorized },
    );
...
test(`${path.basename(
    __filename,
)}: Logged out refresh tokens can no longer be used to refresh/logout`, async t => {
    t.plan(2);
    const client = t.context.databaseClients.childClient;
    const loginResultUserSession1 = await client.query(
        Call('login', 'user@domain.com', 'verysecure'),
